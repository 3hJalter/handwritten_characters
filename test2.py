...
def setup_panel(display):
    ...
    if best_predictions:
        ...
        prediction_status_coordinate = [
            ((725, 505), (830, 505), (0, 0, 255)),
            ((725, 562), (830, 562), (0, 255, 0)),
            ((725, 619), (830, 619), (255, 0, 0))
        ]
        for i in range(len(labels)):
            label_coordinate, acc_coordinate, color = prediction_status_coordinate[i]
            
            cv2.putText(display, labels[i], label_coordinate, cv2.FONT_HERSHEY_SIMPLEX, 0.8, color, 2)
            cv2.putText(display, str(accs[i]), acc_coordinate, cv2.FONT_HERSHEY_SIMPLEX, 0.8, color, 2)
        
        for i in range(len(labels), 3):
            label_coordinate, acc_coordinate, color = prediction_status_coordinate[i]
            
            cv2.putText(display, "_", label_coordinate, cv2.FONT_HERSHEY_SIMPLEX, 0.8, color, 2)
            cv2.putText(display, "_", acc_coordinate, cv2.FONT_HERSHEY_SIMPLEX, 0.8, color, 2)
    
    cv2.putText(display, current_action, (745, 120), cv2.FONT_HERSHEY_SIMPLEX, 0.6, action_colors[current_action], 1)
    
def arrange_crop_rectangle_coordinates(cor1, cor2):
...
def character_crop(event, x, y):
    global bound_rect_coordinates, lbd_coordinate, lbu_coordinate, crop_preview, display, best_predictions
    ...
    if wb_x1 <= x <= wb_x2 and wb_y1 <= y <= wb_y2:
        ...
        if lbd_coordinate is not None and lbu_coordinate is not None:
            bound_rect_coordinates = arrange_crop_rectangle_coordinates(lbd_coordinate, lbu_coordinate)
        elif lbd_coordinate is not None:
            if event is cv2.EVENT_MOUSEMOVE:
                mouse_move_coordinate = (x, y)
                mouse_move_rect_coordinates = arrange_crop_rectangle_coordinates(lbd_coordinate, mouse_move_coordinate)
                top_coordinate, bottom_coordinate = mouse_move_rect_coordinates[0], mouse_move_rect_coordinates[1]
                
                display_copy = display.copy()
                cropped_region = display_copy[top_coordinate[1]:bottom_coordinate[1], top_coordinate[0]:bottom_coordinate[0]]
                ...
                if cropped_rect is not None:
                    display_copy[top_coordinate[1]:bottom_coordinate[1], top_coordinate[0]:bottom_coordinate[0]] = cropped_rect
                    cv2.imwrite("captured/filled.jpg", display_copy)
                    cv2.imshow(window_name, display_copy)

        if bound_rect_coordinates is not None:
            top_coordinate, bottom_coordinate = bound_rect_coordinates[0], bound_rect_coordinates[1]
            crop_preview = display[top_coordinate[1]:bottom_coordinate[1], top_coordinate[0]:bottom_coordinate[0]].copy()
            crop_preview = np.invert(crop_preview)
            best_predictions = predict(model, crop_preview)
            print(best_predictions)
            display_copy = display.copy()
            bound_rect_coordinates = lbd_coordinate = lbu_coordinate = None
            setup_panel(display)
            cv2.imshow(window_name, display)
    elif event is cv2.EVENT_LBUTTONUP:
        lbd_coordinate = lbu_coordinate = None
        cv2.imshow(window_name, display)        
...
model = load_model("D:\Code\Code\Workspace\Python\A_Z Handwritten Data\models\\best_val_loss_model.h5")      
